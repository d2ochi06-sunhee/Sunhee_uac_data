...
slides_data = [
    ...
    },
    {
        ...
        'screen': 'Action 1(Z세대 K-Pop 굿즈/핫플), Action 2(K-뷰티/스킨케어 융합), Action 3(FanPass 재방문 혜택) 카드 화면',
        'action': '목소리에 힘을 주어 현업 상품화 전략을 자신감 있게 발표합니다.',
        'script': '“이에 저희 팀은 2026년을 향한 3대 전략적 액션 플랜(Action Items)을 제언합니다. 10번 슬라이드입니다.\n\nACTION 1: 일본/대만 1020 타깃 \'LCC + 성수동 핫플 + K-Pop 굿즈 2박3일 초밀도 패키지\' 개발\n체재일수가 짧은 일본(-7.6일)과 대만(-6.2일) Z세대를 위해 성수동 로드숍, K-Pop 앨범/굿즈 바우처, 안무 원데이 클래스를 묶은 초밀도 코스를 제공합니다.\n\nACTION 2: 중국 30대 타깃 \'강남 VIP K-뷰티 + 미쉐린 K-푸드 융합 상품\'\n피부과 VIP 스킨케어, 퍼스널 컬러 및 K-뷰티 화장품 굿즈 세트를 결합하여 1인당 지출 단가를 대폭 끌어올려야 합니다.\n\nACTION 3: FanPass K-굿즈 연계 재예약 CRM 실험\n귀국 후 K-Goods 역직구몰 20% 전용 할인 쿠폰과 재방문 시 \'K-시크릿 굿즈 웰컴박스\'를 제공하여 6~12개월 내 재예약을 유도합니다.”',
        'checkpoint': '① 일본/대만 K-Pop 굿즈&체험 2박3일, ② 중국 K-뷰티+미식 융합, ③ 귀국 후 FanPass K-굿즈 연계 재예약 실험.'
    },
    {
        'num': 11,
        'title': '3초 만에 완성하는 K-컬처·굿즈 맞춤 상품기획 솔루션 (Dashboard Solution)',
        'screen': '1-Click K-굿즈 & 패키지 기획서 자동 추출 대시보드(index.html) 시연 화면',
        'action': '자연스러운 미소와 자신감 넘치는 목소리로 "실무 솔루션"임을 강조합니다.',
        'script': '“11번 슬라이드는 앞서 제시한 전략을 실무에서 즉시 실행할 수 있는 \'K-컬처 1030 핀포인트 상품기획 대시보드\'입니다.\n\n그렇다면 현업 여행사 마케터들이 매번 이 방대한 원시 데이터를 직접 분석해야 할까요? 그렇지 않습니다.\n\n저희 대시보드에서는 마케터가 4대 페르소나(체크인 팝스타, VIP 뷰티퀸 등) 중 원하는 타깃을 선택하고 [기획서 생성] 버튼만 누르면, K-Pop 굿즈 바우처($50) 및 K-뷰티 혜택이 포함된 1-Click 상품기획서가 3초 만에 자동 추출됩니다.”',
        'checkpoint': '마케터가 버튼 하나로 K-굿즈 바우처 & 4대 페르소나별 패키지 기획서를 3초 만에 추출할 수 있는 대시보드를 구축했다.'
    },
    {
        'num': 12,
        'title': '분석 결과부터 1-Click K-굿즈 기획서까지 (Product Workflow)',
        'screen': '1. 데이터 인사이트 ➔ 2. 페르소나 핀포인트 ➔ 3. 1-Click 기획서 자동 추출로 이어지는 워크플로우',
        'action': '차분하고 전문성 있는 어조로 데이터와 실무 기획의 연계성을 설명합니다.',
        'script': '“12번 슬라이드는 데이터 분석이 어떻게 실제 여행상품 기획으로 연결되는지 보여주는 3단계 워크플로우입니다.\n\n첫째, 메인 대시보드에서 K-컬처·굿즈 고관여층의 행동 특성을 데이터로 확인합니다.\n둘째, 4대 핵심 페르소나별 선호 굿즈(K-Pop, K-뷰티 등) 및 일정을 매칭합니다.\n셋째, 1-Click 기획서 자동 추출을 통해 현업에서 바로 사용할 파일럿 상품안과 FanPass CRM 쿠폰 혜택을 즉시 완성합니다.”',
        'checkpoint': '데이터 분석 ➔ 페르소나 매칭 ➔ 1-Click K-굿즈 상품기획서 자동 추출로 연결되는 실현 가능한 워크플로우.'
    },
    {
        'num': 13,
        'title': '핵심 제언 및 Q&A (Conclusion & Q&A)',
        'screen': '4대 국가별 맞춤 상품 4종 요약 및 "K-컬처 1030 타깃에 K-굿즈를 결합하자!" 결론 문구',
        'action': '청중을 향해 허리를 숙여 깍듯이 인사하며 밝은 표정으로 마무리합니다.',
        'script': '“마지막 13번 슬라이드입니다.\n\n오늘 분석의 결론은 명확합니다. \'1030 K-컬처 관광객에게 K-굿즈 및 맞춤 혜택을 결합한 핀포인트 여행 상품을 제공하는 것\'입니다.\n\n단순히 통계를 보고하는 수준을 넘어, 분석 결과를 1-Click 실무 상품 기획서로 바꿔주는 당사의 솔루션을 활용한다면, 2026년 대한민국 관광 산업은 도약할 것입니다.\n\n이상으로 발표를 마치겠습니다. 경청해 주셔서 감사합니다.”',
        'checkpoint': 'K-컬처 1030 타깃에 K-굿즈 & 맞춤 기획서 솔루션을 결합해 2026년 관광 성과를 극대화하자며 마무리.'
    }
]
